...
public class ExpiringStateExercise extends ExerciseBase {
	...
	public static class EnrichmentFunction extends KeyedCoProcessFunction<Long, TaxiRide, TaxiFare, Tuple2<TaxiRide, TaxiFare>> {
		...
		// состояния для хранения "одиноких" данных
		...
		public void onTimer(long timestamp, OnTimerContext ctx, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws Exception {
			...
			// таймер сработал, это значит, что Watermark (время в системе)
			// перешагнуло время события, а пара так и не пришла.
			if (fareState.value() != null) {
				// отправляем в поток ненайденных оплат
				ctx.output(unmatchedFares, fareState.value());
				fareState.clear();
			}
			if (rideState.value() != null) {
				// отправляем в поток ненайденных поездок
		...
		public void processElement1(TaxiRide ride, Context context, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws Exception {
			...
			// пришла Поездка. Ищем для нее Оплату
			TaxiFare fare = fareState.value();
			if (fare != null) {
				fareState.clear();
				// если нашли пару, удаляем таймер ожидания, который ставила Оплата
				context.timerService().deleteEventTimeTimer(fare.getEventTime());
				out.collect(new Tuple2<>(ride, fare));
			} else {
				// если пары нет, сохраняем поездку и ставим таймер (ждем её время события)
		...
		public void processElement2(TaxiFare fare, Context context, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws Exception {
			...
			// пришла Оплата. Ищем для нее Поездку
			TaxiRide ride = rideState.value();
			if (ride != null) {
				rideState.clear();
				// если нашли пару, удаляем таймер ожидания, который ставила Поездка
				context.timerService().deleteEventTimeTimer(ride.getEventTime());
				out.collect(new Tuple2<>(ride, fare));
			} else {
				// если пары нет, сохраняем оплату и ставим таймер
